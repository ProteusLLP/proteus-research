...
\begin{equation}
 \Pp_{\alpha,\theta}(Y\in C(X_1,\ldots,X_n))=1-\gamma
...
\label{prop:large_n}
Uniformly for $z$ in compact subsets of $(0,\infty)$,
\begin{equation}
...
\paragraph{Derivation sketch.}
Let the branch level satisfy $a=nd\rho+O(1)$ for fixed $\rho\ge0$. Substitution of the leading residue \eqref{eq:ell_tail} into \eqref{eq:lower_branch_formula}--\eqref{eq:upper_branch_formula} gives the limiting lower-branch probability and upper-branch survival probability
\[
...
\paragraph{Derivation sketch.}
On either branch, substitute the leading residue \eqref{eq:ell_tail} into \eqref{eq:lower_branch_formula} or \eqref{eq:upper_branch_formula} and retain the finite integral $I_n(nd)$. On the upper branch, \eqref{eq:J_z} gives
\[
 J_n\!\left(\frac{z}{n+z}\right)
...
\end{equation}
For comparison, Krishnamoorthy, Mathew and Mukherjee \cite{Krishnamoorthy2008} use the Wilson--Hilferty cube-root approximation. Writing $W_i=X_i^{1/3}$, with sample mean $\bar W$ and sample standard deviation $S_W$, their approximate prediction quantile is
\begin{equation}
 y_p^{\mathrm{KMM}}
 =\left[\max\left(0,
...
For $m\le51$, 180 terms of \eqref{eq:glaser_series} are used in the central region, with the residue expansion \eqref{eq:residue_series} used away from it. The Glaser-type series is used wherever its 160- and 180-term sums agree to the numerical tolerance; residues are added until three successive terms are negligible relative to the accumulated sum. To evaluate residue polynomials stably in standard double precision, the analytic factor $H_{m,r}$ is written as the product of its $m-1$ Gamma-ratio factors. Each factor is normalised and expanded separately, and the resulting short Taylor series are combined by convolution.

For $52\le m\le201$, direct residue coefficient formation becomes ill-conditioned in the moderate tail. The central Glaser series remains stable below $t=5.5$. Above this point the runtime uses a packaged shape-preserving interpolation of $\log\widetilde\ell_m(t)$ on 1,400 logarithmically spaced points through $t=12000$. The grid is generated offline by saddlepoint-tilted pointwise inversion of \eqref{eq:laplace_transform}; independent off-grid checks at representative values of $m$ gave maximum relative density error below $2\times10^{-8}$. Runtime evaluation of this backend uses only standard NumPy/SciPy double-precision arithmetic: it performs no Fourier inversion and has no extended-arithmetic dependency. The offline generator is distributed with the software.
...
For repeated calculation, tabulate $w_{n,p}(d)=\log M_{n,p}(d)$ at every required integer $n$. When several probabilities are required at fixed $(n,d)$, the quadrature nodes, values of $\widetilde\ell_n$ and Jacobian factors are formed once; each further probability then requires only weighted sums and scalar root finding. For each stored $n$, a tensor-product interpolating spline is fitted directly to $w$ using $\log d$ and the logit of $p$ as coordinates, with degree four in the dispersion coordinate and degree five in the probability coordinate. No extrapolation is used outside the tabulated range.

The distributed table contains every integer $n=3,\ldots,50$, together with
\[
 n\in\{55,60,65,70,75,80,90,100\}.
\]
At every stored sample size, the production grid has 50 dispersion values from $10^{-6}$ to 50 and 54 probabilities from 0.001 to 0.999. The packaged density-grid scalar backend is supported through $n=200$ within its resource range $t\le12000$; contour inversion is selected automatically above $n=200$ or when that resource range is exceeded.

For $n\in\{3,5,10,20,50,55,75,100\}$ and log-uniform $d$ on $[0.025,1.5]$, 160 fixed-seed lower-tail checks over $p\in[0.001,0.2]$ gave median, 95th-percentile and maximum relative multiplier errors of $0.0001\%$, $0.0040\%$ and $0.0103\%$. The corresponding 160 upper-tail checks over $p\in[0.8,0.999]$ gave $0.0000\%$, $0.0002\%$ and $0.0039\%$. A separate 300-point holdout split by lower or upper tail and by $d\in[10^{-6},0.025]$, $[0.025,10]$ or $[10,50]$ had maximum error $0.0166\%$ across its six strata. Dense checks found no loss of monotonicity in $p$. Inputs outside $d\in[10^{-6},50]$ or $p\in[0.001,0.999]$ are rejected rather than extrapolated; exact scalar inversion remains preferable outside the validated interpolation region when its density argument remains within the supported range.
